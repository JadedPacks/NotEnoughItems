	...
	public static void giveStack(ItemStack base, int i, boolean infinite) {
		...
		if(hasSMPCounterPart()) {
			...
			for(int given = 0; given < stack.stackSize; ) {
				...
				sendCommand(getStringSetting("command.item"), mc().thePlayer.getCommandSenderName(), Item.getIdFromItem(stack.getItem()), qty, stack.getMetadata(), stack.hasTagCompound() ? stack.getTagCompound().toString() : "", Item.itemRegistry.getNameForObject(stack.getItem()));
				given += qty;
			}
		}
	}
